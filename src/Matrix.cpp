...
namespace algebra {
    ...
    void Matrix<T, Order>::compress() {
        ...
        if constexpr (Order == StorageOrder::ROW_MAJOR){    
            ...
            //std::cout<<"Test, printing the normal row indices: "<<std::endl;
            for (auto& entry : data) {
            ...
            // fill for column major
            // Fill CSR vectors from COOmap
            col_indices.resize(cols + 1); //first element is automatically set to 0.
            std::size_t count = 0; //count of non-zero elements in current row 
            std::size_t col_idx = 0; //last row index
            for (const auto& entry : data) {
                //extracting current element indices and value
        ...
        if constexpr (Order==StorageOrder::ROW_MAJOR){
            if (compressed) {
                ...
                for (std::size_t k = row_indices[i]; k < row_indices[i+1]; ++k){
                    if(col_indices[k] == j){
                        //std::cout<<"Executing compressed () with RowMajor ordering"<<std::endl;
                        return values[k];
                        }
                    }
                //std::cout<<"Executing compressed () with RowMajor ordering"<<std::endl;
                ...
                if (it == data.end()) {
                    //std::cout<<"Executing uncompressed () with RowMajor ordering (returning 0) : element not found."<<std::endl;
                    return 0; // Element not found, assuming default value is 0
                }
                //std::cout<<"Executing uncompressed () with RowMajor ordering (returning value)"<<std::endl;
                return it->second;
            }
        }else{
            if (compressed) {
                ...
                if (it == data.end()) {
                    //std::cout<<"Executing uncompressed () with ColumnMajor ordering"<<std::endl;
                    return 0; // Element not found, assuming default value is 0
                }
                //std::cout<<"Executing uncompressed () with ColumnMajor ordering"<<std::endl;
                return it->second;
